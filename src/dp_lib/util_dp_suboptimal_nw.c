...
void prt(gpointer item) {
    HERE ("%d - %d (%d)", ((struct sCache*)item)->r1, ((struct sCache*)item)->r2, ((struct sCache*)item)->w);
}
...
Constraint_list *ProbaMatrix2CLMAX (Alignment *A, int *ns, int **ls, int NumMatrixTypes, int NumInsertStates, float *forward, float *backward, float thr, Constraint_list *CL) {
  ...
  if ( list_max<list_size) {list_max=list_size;}

  totalProb = ComputeTotalProbability (I,J,NumMatrixTypes, NumInsertStates,forward, backward);

  maxPairs = (CL->S->nseq * (CL->S->nseq-1)) / 2;
  maxCons = CL->maxlib/maxPairs;
  if(CL->minseq.first==s1 && CL->minseq.second==s2) {
    maxCons+=CL->maxlib-(maxCons*maxPairs);
  }
  
  pCache = (struct sCache*) malloc(list_max * sizeof(struct sCache));
  ...
  int col_list[I+J]; // col_list2[I+J];
  memset(col_list, 0, sizeof(col_list));
  //memset(col_list2, 0, sizeof(col_list2));
  int col_n = (int)ceil((float)maxCons/(float)(MAX(I,J)+1));
  ...
  while(maxCons>0 && list_n>0) {
	...
	for (i=0; i<list_n; i++) {
		...
		if(col_list[((struct sCache*)temp)->r1]<(col_n) && maxCons>0) { //&& col_list2[((struct sCache*)temp)->r2]<(col_n)
			col_list[((struct sCache*)temp)->r1]++;
			//col_list2[((struct sCache*)temp)->r2]++;
			entry[SEQ1]=s1;
			entry[SEQ2]=s2;
			entry[R1]  =((struct sCache*)temp)->r1;
			entry[R2]  =((struct sCache*)temp)->r2;
			entry[WE]  =((struct sCache*)temp)->w;
			entry[CONS]=1;
			add_entry2list (entry,A->CL);
			temp = NULL;
			maxCons--;
