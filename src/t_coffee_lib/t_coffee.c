		...
		overaln_P3=0;
	       ...
	       if (!strm (dump, "no"))
		 {
	       ...
	       get_cl_param(\
			    ...
			    /*OPTIONAL?*/ OPTIONAL       ,\
			    /*MAX Nval*/  2             ,\
			    /*DOC*/       "Maximum value of pair residues in the library"           ,\
			    /*Parameter*/ &maxlib         ,\
			    /*Def 1*/    "-1",\
                             /*Def 2*/    "NULL",\
			    /*Min_value*/ "any"          ,\
...
/*******************************************************************************************************/
		...
		* Note that when running master, the library must be relaxed in order to compensate for missing pairs
		...
		int minimum = 999;
		for(int x=0; x<S->nseq; ++x) {
		  for(int y=0; y<S->nseq; ++y) {
			if(minimum>(float)(CL->DM)->distance_matrix[x][y] && (float)(CL->DM)->distance_matrix[x][y]!=0) {
			  minimum = (float)(CL->DM)->distance_matrix[x][y];
			  CL->minseq = std::make_pair(x,y);	
			}
		  }
		}
	        //fprintf(stdout, "%d-%d: %d\n", CL->minseq.first, CL->minseq.second, minimum);
